...
| Claude (Anthropic) | 웹서치 기반 팩트체크 판정 | console.anthropic.com |
...
- Claude API는 브라우저發 직접 호출을 기본 차단하므로, `anthropic-dangerous-direct-browser-access` 헤더를 명시적으로 보냅니다. 이는 확장 프로그램이 API 키를 다루는 유일한 지점(background.js)에서만 사용됩니다.

## 예상 비용

영상 1건 분석 기준 100원 미만 (댓글 분류: DeepSeek 수 원, 팩트체크 3~5건: Claude 수십 원, 원본 검색: 이미지 3장 거의 0원). 정확한 단가는 각 API 제공사의 최신 요금표를 확인하세요.
