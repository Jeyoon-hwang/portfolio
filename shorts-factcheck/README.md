...
**콜드 로드 첫 영상이 실패하던 이유 = BotGuard가 아직 준비 안 됨 (트리거로는 해결 불가).**
진단 로그로 확정했다. 페이지를 처음 연 직후 `/youtubei/v1/player` 요청을 **6번이나 봤고 본문도 전부
정상적으로 읽혔는데, 그중 pot이 들어있는 건 하나도 없었다.** 즉 "우리가 요청을 못 본" 문제가 아니라
**유튜브 자신조차 아직 pot 없이 요청을 보내고 있는** 상태였다 — BotGuard 챌린지가 끝나기 전이기 때문이다.

이게 중요한 이유: 한때 "플레이어에 같은 영상을 다시 물리거나(`cueVideoById`) 강제로 스크롤해서 player
요청을 유발하면 되지 않나"를 검토했는데, **트리거를 아무리 더 해도 소용없다는 게 이 로그로 증명됐다.**
필요한 건 트리거가 아니라 시간이다. (덧붙여 pot은 videoId 바인딩이라, 스크롤로 다음 영상 pot을 얻어봐야
현재 영상엔 못 쓴다.)

그래서 `sfc-pot-result`에 **`everSeenPot`**(이 세션에서 pot을 하나라도 본 적 있는지)을 실어 보내고,
아직 하나도 없으면 = BotGuard 진행 중이라는 뜻이므로 pot 대기 시간을 2.5초 → 10초로 늘린다. 반대로
pot을 본 적은 있는데 이 영상 것만 없는 경우엔 **연장하지 않는다** — 그건 기다려도 안 생기는 상황이라
괜히 파이프라인만 늦어지기 때문이다. 이 비용은 세션당 첫 영상에서 한 번만 발생한다.
...
**그리고 남의 pot으로는 아예 요청하지 않는다.** 실측에서 다른 영상 pot을 붙여 보낸 요청은 3번 모두
200 OK + 0바이트였고 성공한 적이 한 번도 없다 — pot이 videoId 바인딩이니 당연한 결과다. 실패가 확정된
요청을 굳이 보내면 왕복 시간만 버리고 `pot: yes인데 본문이 비었다`는 헷갈리는 로그만 남으므로, 이 영상
pot이 없으면 **다운로드를 건너뛰고 바로 캡처 단계로 넘어간다.** (`exp=xpe`가 없어 pot이 필요 없는
트랙은 예전처럼 그냥 받는다.)

**그래서 대기 시간은 "이 영상 pot이 없을 때" 무조건 넉넉히 준다.** 처음엔 "다른 영상 pot은 있는데
이 영상 것만 없으면 기다려도 안 생긴다"고 보고 2.5초 만에 포기하게 했는데, 실측에서 그게 틀렸다 —
다른 영상 pot이 이미 있는 상태에서도 이 영상 pot만 늦게 도착하는 경우가 흔했다(유튜브가 그 영상의
player 요청을 아직 안 끝냈을 뿐이다). **남의 pot으로 대신 요청하면 100% 빈 본문이므로 일찍 포기해서
얻는 이득이 아예 없다.** 지금은 이 영상 pot이 없으면 5초(콜드 로드로 판단되면 10초)까지 기다린다.
이미 pot이 있으면 질의 즉시 응답이 오므로 평상시 속도에는 영향이 없다.
